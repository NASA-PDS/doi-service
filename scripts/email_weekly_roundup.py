...
def get_email_content_template(template_filename: str = "email_weekly_roundup.jinja2"):
    template_filepath = resource_filename(__name__, template_filename)
...
def run(db_filepath: str, sender_email: str, receiver_email: str) -> None:
    ...
    msg = prepare_email_message(sender_email, receiver_email, target_week_begin.date(), last_date_of_week,
                                modified_doi_records)
...
if __name__ == '__main__':
    ...
    run(db_filepath, sender_email_address, receiver_email_address)
