...
def draft_action_run(node_value, input_value, lid=None):
    ...
    o_doi_label = _action.run(input=input_value, node=node_value, submitter="my_user@my_node.gov", force=True)
...
def reserve_action_run(node_value, input_value, lid=None):
    ...
    o_doi_label = _action.run(
        input=input_value, node=node_value, submitter="my_user@my_node.gov", dry_run=True, force=True
    )
...
def release_action_run(node_value, input_value):
    try:
        ...
        released_doi_str = release_action.run(
            input=input_value, node=node_value, submitter="my_user@my_node.gov", force=True
        )
...
def given_valid_action_input(context, input_value):
    ...
    context.input_value = (
        input_value  # Don't forget to set the input_value in context to be available for other functions.
    )
...
def when_create_draft_impl(context, node_value):
    ...
    try:
        ...
        new_draft_output = draft_action_run(
            node_value, context.input_value, lid=context.random_lid if hasattr(context, "random_lid") else None
        )
...
def step_then_osti_doi_label_is_created_impl(context, node_value, input_value):
    ...
    try:
        if not hasattr(context, "output_files"):
            context.output_files = []
        reserve_ouput_file = reserve_action_run(
            node_value, input_value, lid=context.random_lid if hasattr(context, "random_lid") else None
        )
...
def step_doi_label_is_submitted_impl(context):
    ...
    if g_submit_flag:
        doi, response_str = DOIOstiWebClient().submit_content(payload=etree.tostring(out_root))
    else:
...
def when_reference_is_drafted_from_impl(context, node_value, input_subdir):
    ...
    if not hasattr(context, "output_files"):
        context.output_files = []
    new_draft_file = draft_action_run(
        node_value, input_dir, lid=context.random_lid if hasattr(context, "random_lid") else None
    )
...
def step_reference_is_reserved_at_input_impl(context, node_value, input_value):
    ...
    if not hasattr(context, "output_files"):
        context.output_files = []
    context.output_files.append(
        reserve_action_run(node_value, input_dir, lid=context.random_lid if hasattr(context, "random_lid") else None)
    )
...
def submit_osti_record(context, node_value):
    try:
        ...
        logger.info(
            f"record in file {context.output_files[-1]} submitted from output index {len(context.output_files)}"
        )
    except CriticalDOIException as e:
        context.exception_msg = str(e)
