...
class DOIPDS4LabelUtil:
    ...
    def ProcessIAD2ProductLabelMetadata(self,dict_fixedList, dict_ConditionData, eachFile, FileName, xmlContent=None):
        function_name = self.m_module_name + 'ProcessIAD2ProductLabelMetadata:'
        global m_debug_mode
        ...
        dict_namespaces = self.m_doiGeneralUtil.ReturnNameSpaceDictionary(f_debug, debug_flag, eachFile,xmlContent)
        if m_debug_mode:
        ...
        try:  
            ...
            if os.path.isfile(eachFile):
                ...
                if xmlContent is not None:
                    ...
                    string_to_walk = self.m_doiGeneralUtil.DecodeBytesToString(xmlContent);   # The content may be a bunch of bytes.
            ...
            for event, element in context:
                ...
                if (element.tag == objPubYear_uri) or (element.tag == objPubDate_uri):
                    if (event == "start"):
                        ...
                        dict_ConditionData[FileName]["publication_date"]  = self.m_doiGeneralUtil.ReturnDOIDate(f_debug,debug_flag,PubDate_value); # Conver to DOI format
                        dict_ConditionData[FileName]["date_record_added"] = self.m_doiGeneralUtil.ReturnDOIDate(f_debug,debug_flag,PubDate_value); # Conver to DOI format
    ...
    def ProcessDOIMetadata(self,dict_configList, dict_fixedList, dict_ConditionData, FileName, list_keyword_values, DOI_filepath,publisher_value=None,contributor_value=None):
        function_name = self.m_module_name + 'ProcessDOIMetadata:'
        global m_debug_mode;
        #m_debug_mode = True;
        if m_debug_mode:
        ...
        for key, value in dict_value.items():
            ...
            xmlDOI_Text = self.m_doiOutputUtil.PopulateDOIXMLWithValues(dict_fixedList, xmlDOI_Text, attr_xpath, value)                          
        #------------------------------                                                                                                 
        ...
        xmlDOI_Text = self.m_doiOutputUtil.PopulateDOIXMLWithValues(dict_fixedList, xmlDOI_Text, attr_xpath, publisher_value)
        if m_debug_mode:
        ...
        xmlDOI_Text = self.m_doiOutputUtil.PopulateDOIXMLWithValues(dict_fixedList, xmlDOI_Text, attr_xpath, contributor_value)
        ...
        #  -- each value must be separated by semi-colon
        #------------------------------   
        keyword_values = self.m_doiGeneralUtil.ReturnKeywordValues(dict_configList, list_keyword_values);
        ...
        xmlDOI_Text = self.m_doiOutputUtil.PopulateDOIXMLWithValues(dict_fixedList, xmlDOI_Text, attr_xpath, keyword_values)                          
    ...
    def ProcessParentNode(self,previous_parent,pds_uri_string,element,event):
        function_name = self.m_module_name + 'ProcessParentNode:';
