...
class DOIInputUtil:
    ...
    def AggregateReserveDOI(self,DOI_directory_PathName,i_filelist):
        function_name = self.m_module_name + 'AggregateReserveDOI:';
    ...
    def ShowColumnNamesInXLS(self,xl_sheet):
        function_name = self.m_module_name + 'ShowColumnNamesInXLS:';
        #------------------------------
    ...
    def ParseSXLSFile(self,appBasePath,i_filepath,dict_fixedList=None, dict_configList=None, dict_ConditionData=None):
        # Function receives a URI containing SXLS format and create one external file per row to output directory.
        function_name = self.m_module_name + 'ParseSXLSFile:';
        global m_debug_mode
        global f_log
        #m_debug_mode = True;
        o_doi_label = None;
        ...
        if (num_cols < EXPECTED_NUM_COLUMNS):
            ...
            o_aggregated_DOI_content = self.AggregateReserveDOI(DOI_directory_PathName,i_filelist);

        return(o_num_files_created,o_aggregated_DOI_content);

    def ParseCSVFile(self,appBasePath,i_filepath,dict_fixedList=None, dict_configList=None, dict_ConditionData=None):
...
if __name__ == '__main__':
    ...
    i_filepath = '.' + os.path.sep + 'input' + os.path.sep + 'DOI_Reserved_GEO_200318.csv';
