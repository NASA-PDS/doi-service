...
class TransactionBuilder:
    ...
    m_transaction_logger = None # A logger to write transacton to disk and to database.
    ...
    def __init__(self):
        ...
        self.m_transaction_logger = TransactionLogger(self.m_doi_database)

    def get_transaction(self):
        return self.m_transaction 

    def get_transaction_logger(self):
        return self.m_transaction_logger 
    ...
    def prepare_transaction(self,target_url,node_id,action_type,submitter_email,doi_fields,output_content=None,web_response=None):
        ...
        if 'dois' in doi_fields:
            self.m_transaction = Transaction(target_url,node_id,action_type,submitter_email,doi_fields['dois'])
            for ii in range(0,len(doi_fields['dois'])):
                self.m_transaction.add_field('output_content',output_content,ii)
                # If web_response is not provided, we don't know the status yet so set it to 'unknown'.
                if web_response is None:
                    self.m_transaction.add_field('status','unknown',ii)
        else:
            # If the key 'dois' is not in doi_fields, we assume it is a list of one dictionary with many fields.

            self.m_transaction = Transaction(target_url,node_id,action_type,submitter_email,[doi_fields])
            self.m_transaction.add_field('output_content',output_content,0)
            if web_response is None:
                # If there is no web_response (i.e no interaction with the data server) and the 'draft' action was done,
                # set to 'pending', otherwise, set to 'unknown' status.
                if action_type == 'draft':
                    self.m_transaction.add_field('status','Pending'.lower(),0)
                else:
                    self.m_transaction.add_field('status','unknown',0)

            # The field 'identifier' contains 'urn:nasa:pds:insight_cameras::1.0' so we parse for 'lid' and 'vid' fields.
            ...
            if len(identifier_tokens) < 2:
                ...
                exit(1)
            self.m_transaction.add_field('lid',identifier_tokens[0],0)
            self.m_transaction.add_field('vid',identifier_tokens[1],0)

        logger.debug(f"self.m_transaction.get_transaction() {self.m_transaction.get_transaction()}")

        return (self.m_transaction_logger,self.m_transaction)
